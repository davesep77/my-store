...
export const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API Latency (connecting to MySQL/XAMPP backend)
    await new Promise(resolve => setTimeout(resolve, 1500));
    
    if (isLogin) {
      // Validate specific admin credentials requested
      if (formData.username === 'dave' && formData.password === '#d0911136479E') {
        const adminUser: UserType = {
          id: '1',
          username: 'dave',
          fullName: 'Dave Admin',
          email: 'dave@mystore.com',
          role: 'admin'
        };
        onLogin(adminUser);
      } else {
        setError('Invalid username or password. Please use authorized credentials.');
        setIsLoading(false);
      }
    } else {
