...
export const Settings: React.FC<SettingsProps> = ({ settings, onUpdate }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto animate-in fade-in duration-500">
      <div className="flex items-center gap-4 mb-8 bg-white p-2 rounded-2xl w-fit shadow-sm border border-gray-100">
        <button
          onClick={() => setActiveTab('general')}
          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${activeTab === 'general' ? 'bg-[#8E54E9] text-white shadow-lg shadow-purple-200' : 'text-gray-500 hover:bg-gray-50'
            }`}
        >
          <Globe size={18} />
          General Settings
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-xl font-bold transition-all ${activeTab === 'subscription' ? 'bg-[#8E54E9] text-white shadow-lg shadow-purple-200' : 'text-gray-500 hover:bg-gray-50'
            }`}
        >
          <Coins size={18} />
      ...
      {activeTab === 'general' ? (
        ...
        <Subscription settings={settings} onUpdate={onUpdate} />
      )}
    </div>
  );
};
