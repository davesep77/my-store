      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden print:border-none print:shadow-none">
        ...
        <div className="p-10 pb-6 no-print">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-3xl font-black text-gray-800 tracking-tight">Report</h2>
            <div className="flex bg-gray-100 p-1 rounded-xl">
              <button
                onClick={() => setViewMode('detailed')}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${viewMode === 'detailed' ? 'bg-white text-[#8E54E9] shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
              >
                Detailed View
              </button>
              <button
                onClick={() => setViewMode('summary')}
                className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${viewMode === 'summary' ? 'bg-white text-[#8E54E9] shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
              >
                Monthly Summary
              </button>
            </div>
                    ...
                    <React.Fragment key={t.id}>
                      ...
                      >
                        ...
                        <td className="px-10 py-5 text-right">
                          <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md shadow-sm border ${t.type === 'sale' ? 'text-red-600 bg-red-50 border-red-100' :
              ...
              {Object.entries(
                transactions
                  ...
                  .reduce((acc, t) => {
                    ...
                    if (!acc[key]) acc[key] = {};
                    const monthKey = d.toLocaleString(settings.locale, { month: 'long' });
                    if (!acc[key][monthKey]) acc[key][monthKey] = { count: 0, total: 0, items: [] };

                    acc[key][monthKey].count += 1;
                    acc[key][monthKey].total += (t.quantity * t.unitPrice);
                    acc[key][monthKey].items.push(t);
                    return acc;
                  }, {} as Record<string, Record<string, { count: number, total: number, items: Transaction[] }>>)
              ).sort((a, b) => Number(b[0]) - Number(a[0])).map(([year, months]) => (
                <div key={year} className="mb-12">
                  <h3 className="text-4xl font-black text-gray-200 mb-6 flex items-center gap-4">
                    {year}
                    <div className="h-px bg-gray-100 flex-1"></div>
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {Object.entries(months).map(([month, data]) => (
                      <div key={month} className="bg-white border border-gray-100 p-8 rounded-[2rem] shadow-lg shadow-gray-100 hover:shadow-xl transition-all group">
                        <div className="flex justify-between items-start mb-6">
                          <div>
                            <h4 className="text-xl font-black text-gray-800">{month}</h4>
                            <p className="text-xs font-bold text-gray-400 uppercase tracking-wider mt-1">{data.count} Transactions</p>
                          </div>
                          <div className="w-12 h-12 bg-purple-50 rounded-2xl flex items-center justify-center text-[#8E54E9] group-hover:scale-110 transition-transform">
